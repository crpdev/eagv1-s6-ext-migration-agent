...
# Configure logging to file only (no stdout to avoid interfering with stdio transport)
...
@mcp.tool(name="modUpgradeAll", description="Upgrade all projects using specified recipe")
def mod_upgrade_all(BaseModel) -> dict:
    params: str
    """Upgrade all projects using specified recipe."""
    logger.info(f"Starting modUpgradeAll with recipe: {params}")
